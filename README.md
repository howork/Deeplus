...
앞 네 명령은 한국어 참조서와 튜토리얼이 정확 정본에 결합됐는지,
목차·상태·설명량·예제·연습·링크와 생성 결정성이 유지되는지 검사한다.
workspace 검증기는 registry, schema, authority, 경로와 source-tree
폐쇄성을 검사한다. Cargo 명령은 저장소의 Rust 골격만 검사하며 제품
적합성 증거가 아니다.
