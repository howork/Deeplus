...
5. 문서 변경은 해당 deterministic projection을 먼저 검증한다.
...
6. `python tools/validators/validate_workspace.py`, `cargo check --workspace`, 관련 test를 실행한다.
7. 실행하지 않은 product lane은 `NOT_RUN`으로 남긴다.
8. 보고서는 영문 파일명과 역할 prefix를 사용하고, 상세 분석·대안·acceptance test·handoff를 포함한다.
