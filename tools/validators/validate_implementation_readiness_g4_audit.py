...
def validate(root: Path) -> list[str]:
    ...
    require(metadata.get("base_count") == 463, "BASE_STATUS_COUNT")
    ...
    require(derived.get("bound_direct_cells") == 3709, "DERIVED_DIRECT")
    require(derived.get("bound_delegated_cells") == 4, "DERIVED_DELEGATED")
    require(derived.get("not_applicable_cells") == 508, "DERIVED_NA")
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("G4 IMPLEMENTATION READINESS AUDIT: PASS")
    print("- readiness gates: 5/5 PASS_E2")
    print("- target rows: 469")
