...
def validate(root: Path, metadata: dict[str, Any], rows: list[dict[str, Any]]) -> list[str]:
    ...
    require(len(overlay_bindings) == 121, "OVERLAY_BINDING_EXACT_TOTAL_121")
    ...
    # R59-R62 are bounded pending-generation overlays. Make their exact E2 entries
    ...
    require(
        (direct, delegated, na, blocked) == (2458, 3, 502, 1258),
        "R62_EXACT_POST_OVERLAY_COUNTS",
    )
