...
- exact numeric constant가 target type에 적응하는 경계를 이해한다.
...
소스 숫자 뒤에 `i32`, `u8`, `f32` 같은 type suffix를 붙이면 lexical
spelling과 type 책임이 결합되고 overload가 width-fit을 추측하기 쉽다.
Deeplus는 숫자 상수를 먼저 exact value로 읽고 선언·parameter 같은 유효한
target에서만 domain을 정한다. 정적 연산은 “적당히 작은/큰 타입”을
고르지 않으며 operand domain과 overflow 책임을 HIR/MIR에 남긴다.

## 5. 핵심 모델

- `()`는 유일한 `Unit` 값이다.
- `true`, `false`는 `Bool`이다.
- target 없는 integer는 기본 `Int`다.
- `UInt`는 별도 64-bit unsigned 기본 의미 domain이며 `UInt64` 별칭이 아니다.
- target 없는 decimal real은 `Float64`다.
...
target이 없는 `42`는 `Int`로 default된다. `exact`, `mask`, `compact`는
각 선언 target이 먼저 고정되어 있고 literal 값이 정확히 표현 가능하므로
그 domain에 제한적으로 적응한다. 이것은 signedness나 width 사이의 일반
implicit conversion이 아니다. `ratio`의 `Float`는 정규화 뒤 정확히
`Float64`다.
...
literal을 만나면 token spelling과 exact value를 읽고, 유효한 target이
...
제거된 suffix-shaped token은 identifier와 둘로 쪼개지 않는다.

<!-- deeplus-example: illustrative; surface: CURRENT; expected: REJECT; diagnostic: NUMERIC_TYPE_SUFFIX_REMOVED; product: NOT_RUN -->
```deeplus
let legacy = 42i32
// NUMERIC_TYPE_SUFFIX_REMOVED: `let legacy: Int32 = 42`로 고친다.
```

scanner는 `42i32` 전체를 maximal-munch한 뒤 진단 하나를 내고 admitted
CST/AST/HIR를 만들지 않는다. radix/exponent, Rational `<p/q>`, Measure
unit, floating-look imaginary `4.0i`는 서로 다른 suffix domain이라 유지된다.
...
- exact domain은 선언·parameter·명시 API 경계에서 고정한다.
...
1. **따라 하기:** `Int`, `Int16`, `Float32` 값을 각각 type annotation과
   suffix 없는 literal로 선언한다.
2. **빈칸 완성:** suffix 없는 소수 `2.5`의 기본 타입은 `___`이고,
   `let value: Float32 = 2.5`의 target 타입은 `___`이다.
...
- numeric type suffix는 제거되었고 target이 exact width/domain을 고정한다.
