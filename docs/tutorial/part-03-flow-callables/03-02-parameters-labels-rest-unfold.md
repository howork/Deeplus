...
- suffix collect(`..`, `**`)와 prefix unfold(`*`)의 방향을 구분한다.
...
이 장에서 `NamedPack<rho>`는 호출 한 번에만 존재하는 유한한 정적 label
row라는 최소 직관만 사용한다. `options**`는 고정 parameter에 결합되지
않은 named argument를 이 row로 모은다. `NamedPack`은 `Record`, `Map`,
schema 값과 서로 바꿀 수 없으며 자세한 값 모델은 Part 5에서 배운다.
`borrow`/`move`는 값을 읽거나 소유권을 넘기는 parameter mode라는
...
Deeplus는 positional repeated channel과 static-named `NamedPack` channel을 서로
...
- `options**`는 유일하고 마지막인 named-rest parameter다.
- 호출의 `*value`는 owner-bounded structural unfold다. source shape가
  positional인지 static-named인지는 overload보다 먼저 확정된다.
- 함수 타입은 positional residue를 `T..`, static-named residue를
  `NamedPack**`로 보존한다.
...
`NamedPack**`로 남는다. body의 `options`는 유한하고 nonescaping인
`NamedPack<rho>`이며 일반 `Record`로 지워지지 않는다.

고정 suffix parameter를 rest 뒤에 둘 수 있지만 default를 붙일 수 없다.
명시 label을 먼저 결합하고, 오른쪽에서 fixed suffix를 예약한 다음,
왼쪽 fixed prefix를 채우며 남은 positional actual만 rest로 들어간다.

```deeplus
private def frame(prefix: String, fields..: String, checksum: UInt) -> Frame = {
    return Frame!(prefix: prefix, fields: fields, checksum: checksum)
}

let packet = frame("D+", "alpha", "beta", 91)
```

여기서 `91`은 오른쪽 fixed suffix인 `checksum`에 먼저 예약되고 `alpha`,
`beta`만 `fields`에 모인다. `checksum: UInt = 0`처럼 rest 뒤 fixed
parameter에 default를 두면 call shape가 둘로 읽히므로 거부된다.

### 6.1 named-rest의 required-field 계약

```deeplus
private def connect(options** requires {
    timeout: Duration
    retries: Int
}) -> Connection = {
...
이 `requires { ... }`는 `options**`가 반드시 포함할 정적 label과 타입만
기술한다. callable 전체의 `requires PredicateExpr` 계약과 AST owner가
다르며, 값 predicate나 동적 key 검사를 넣을 수 없다. duplicate label,
Map feed, runtime-computed label, escape 저장은 모두 거부된다.
...
### 6.2 parameter Pattern은 call shape를 바꾸지 않는다
...
타입을 만들 때도 `T..`와 `NamedPack**` residue를 지우지 않는다.

미니 사례에서 `command("copy", *args, **options)`의 `*`는 positional
source를 펼치고 `**`는 static-named source를 펼친다. 서로 다른 prefix가
channel 종류를 소스에서 고정하므로 expected formal이나 overload 결과가
그 종류를 추측하지 않는다. checker가
source shape를 먼저 봉인하고 그 뒤 call candidate를 고른다. Map key는
NamedPack label로 승격되지 않는다.

## 7. 허용·거부·경계 사례

formal은 `options**`, call unfold는 `**options`여야 한다. named-rest는
정적으로 닫힌 `NamedPack`만 받으며 `Map`을 펼쳐 label proof를 대신할 수
없다. `*value`를 괄호 밖 일반 prefix expression으로 쓰는 것도 거부된다.
label을 formal과 다른 순서로 적어도 expression
...
- 열린 이름 설정이 정말 필요할 때만 최종 `NamedPack**`를 둔다.
...
3. **스스로 설계하기:** 고정 parameter와 `NamedPack**`를 함께 쓰는 API를
   설계하고, 왜 `Map`을 받지 않는지 설명한다.

## 11. 빠른 복습

- label은 runtime 문자열이 아니라 정적 call identity다.
- suffix `..`/`**`는 collect, prefix `*`는 owner-bounded unfold다.
- named-rest는 하나, 마지막, 유한한 `NamedPack`이다.
