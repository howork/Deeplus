...
| `ASSOCIATED_REQUIREMENT_UNRESOLVED` | `checker` | `error` | `active` | Associated type/value/function requirement cannot be admitted or resolved in this witness or constraint environment. |
...
| `EFFECT_ERROR_ROW_POLYMORPHISM_NOT_ADMITTED` | `checker` | `error` | `active` | The quantified effect/error row constraints do not produce one principal closed substitution within the declared scope. |
| `EFFECT_ROW_SUBSUMPTION_NOT_ADMITTED` | `checker` | `error` | `active` | The implementation and required effect rows cannot be normalized or do not satisfy the relation required by this trait witness, class override, or function value. |
...
| `AssociatedRequirementAdmitted` | AssociatedRequirementAdmitted | R9 typed algorithm; product NOT_RUN. | `STATIC_REFERENCE_VALIDATOR_PASS` |
