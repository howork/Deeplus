...
| 프로필 | 생성 규칙 수 | 역할 |
|---|---:|---|
| `LEXICAL` | 91 | 소스 문자, 토큰, 스캐너 결과 및 비의미 토큰 |
| `STABLE` | 539 | 현행 구조 구문 |
| `PREVIEW` | 13 | 명시적 gate를 거쳐 활성화할 수 있는 Preview 루트 |
| **합계** | **643** | 대소문자를 구별하는 정확한 생성 규칙 정의 |
...
돌아올 때도 `CURRENT` fence를 명시한다. 이 규칙은 gated Preview와
비활성 Preview가 같은 절에 섞여 현행처럼 보이는 것을 막는다.
...
생성된 커버리지의 `100%`도 같은 경계를 가진다. 그것은 레지스트리와
정본 행이 빠짐없이 투영되었다는 byte/identity 완전성 증거이지, 각
기능의 정적 판정 알고리즘, 평가 순서, 소유권 이전, 실패 원자성,
cleanup 및 백엔드 관찰이 모두 닫혔다는 의미 완전성 증거가 아니다.
본문은 닫힌 규칙을 단계별로 설명하고, 대상 실행으로 확인되지 않은
층은 `PRODUCT_NOT_RUN` 또는 해당 Preview fence에 그대로 둔다.
